package tk.duelnode.gameserver.util;
...
public class WorldEditUtil {
    ...
    public void paste(Clipboard clipboard, Vector location, World world) {
        ...
        Vector offs = clipboard.getRegion().getCenter().subtract(clipboard.getOrigin());
        offs = offs.add(clipboard.getDimensions().divide(2));
        offs = offs.subtract(clipboard.getDimensions());

        try {
            Operations.complete(holder.createPaste(editSession, holder.getWorldData()).to(location.subtract(offs)).build());
